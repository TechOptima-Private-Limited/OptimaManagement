...
def handle_resource_owner_approval(request, request_id, token, action):
    try:
        ...
        if access_request.status not in ['APPROVER_APPROVED', 'APPROVER_REJECTED']:
            print(f"DEBUG (RO): Request already processed or in wrong state (Status: {access_request.status})")
            return render(request, 'resource_management/approval_success.html', {
                'action': 'error',
                'message': 'This request has already been processed or is not ready for final approval.',
                'ticket': access_request.ticket_number
            })

        # 2. Validate token
        if access_request.approval_token != token:
            print(f"DEBUG (RO): Token mismatch! Received '{token}' but stored '{access_request.approval_token}'")
            return render(request, 'resource_management/approval_success.html', {
                'action': 'error',
                'message': 'Invalid or expired approval link.',
                'ticket': access_request.ticket_number
        ...
        # 3. Validate expiry
        if access_request.approval_token_expiry and access_request.approval_token_expiry < timezone.now():
            print(f"DEBUG (RO): Token expired at {access_request.approval_token_expiry}")
            return render(request, 'resource_management/approval_success.html', {
                'action': 'error',
                'message': 'Approval link has expired.',
                'ticket': access_request.ticket_number
...
def send_approval_confirmation_email(access_request, action, approver_type):
    ...
    try:
        ...
        result = send_email_notification(
            access_request,
            subject,
            'approval_confirmation_email.html',
            context,
            [approver_email],
            is_reply=True
        )
...
def handle_approval(request, request_id, token, action):
    try:
        ...
        log_file = os.path.join(settings.BASE_DIR, 'approval_debug.log')
        with open(log_file, 'a') as f:
            f.write(f"\n--- {timezone.now()} ---\n")
            f.write(f"Ticket: {access_request.ticket_number}\n")
            f.write(f"Received token: '{token}'\n")
            f.write(f"Stored token:   '{access_request.approval_token}'\n")
            f.write(f"Current Status: '{access_request.status}'\n")
            f.write(f"Token Expiry:   '{access_request.approval_token_expiry}'\n")

        # 1. Check if already processed FIRST
        if access_request.status not in ['PENDING', 'APPROVAL_REQUIRED']:
            with open(log_file, 'a') as f: f.write("Result: ALREADY PROCESSED\n")
            return render(request, 'resource_management/approval_success.html', {
                'action': 'error',
                'message': 'This request has already been processed.',
                'ticket': access_request.ticket_number
        ...
        # 2. Validate token
        if access_request.approval_token != token:
            with open(log_file, 'a') as f: f.write("Result: TOKEN MISMATCH\n")
            return render(request, 'resource_management/approval_success.html', {
                'action': 'error',
                'message': 'Invalid or expired approval link.',
                'ticket': access_request.ticket_number
        ...
        # 3. Validate expiry
        if access_request.approval_token_expiry and access_request.approval_token_expiry < timezone.now():
            with open(log_file, 'a') as f: f.write("Result: EXPIRED\n")
            return render(request, 'resource_management/approval_success.html', {
                'action': 'error',
                'message': 'Approval link has expired.',
                'ticket': access_request.ticket_number
        ...
        with open(log_file, 'a') as f: f.write("Result: SUCCESS (Validating...)\n")
...
def send_it_support_notification(access_request, action):
    ...
    try:
        ...
        if access_request.request_type == 'IT':
            ...
            if action == 'approved':
                template = 'resource_team_approved_notification.html' 
            else:
