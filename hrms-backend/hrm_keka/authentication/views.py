...
def get_captcha(request):
    ...
    try:
        ...
        # Ensure image_url is an absolute URL if possible
        ...
        logger.error(f"Captcha generation failed: {str(e)}", exc_info=True)
        return Response({
            'error': 'Internal server error during captcha generation',
            'detail': str(e) if not request.path.startswith('/api/auth/captcha') else 'Check server logs'
