...
from .models import (
    AssetType, Asset, AssetAssignment, AssetHistory, AssetReturn, 
    OffboardingAssetReturn, EmployeeStatus, AssetRepair
)
from .serializers import (
    AssetTypeSerializer,
    AssetSerializer,
...
@login_required
def asset_summary(request):
    """Display asset summary statistics"""
    by_type = Asset.objects.values('asset_type__name').annotate(total=Count('id')).order_by('asset_type__name')
    by_status = Asset.objects.values('status').annotate(total=Count('id')).order_by('status')
    by_employee = AssetAssignment.objects.values('employee__username').annotate(total=Count('assets')).order_by('employee__username')
...
@login_required
def return_assets_form(request):
    """Handle asset return form submission"""
    if 'selected_assignments' not in request.session:
        messages.error(request, "No assignments selected for return.")
        return redirect('admin:assets_assetassignment_changelist')
...
class AssetTypeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        has_broad_access = (
            ...
            user.is_staff or
            user.has_perm('assets.view_assettype') or
        ...
        # Limited access: only asset types they manage
        return qs.filter(asset_team_email=user.email)
...
class AssetViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        has_broad_access = (
            user.is_superuser or
            user.is_staff or
            user.has_perm('assets.view_asset') or
            permission_level >= PERMISSION_LEVELS['SENIOR_LEADER'] or
        ...
        if has_broad_access:
            # Admins/Managers see everything unless they explicitly ask for 'mine'
            if self.request.query_params.get('mine') == 'true':
                return qs.filter(assignments__employee=user, status='ASSIGNED').distinct()
            return qs
        ...
        asset_type_ids = AssetType.objects.filter(asset_team_email=user.email).values_list('id', flat=True)
        return qs.filter(
            Q(asset_type__id__in=asset_type_ids) | 
            Q(assignments__employee=user)
        ).distinct()
...
class AssetAssignmentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        has_broad_access = (
            user.is_superuser or
            user.is_staff or
            user.has_perm('assets.view_assetassignment') or
            permission_level >= PERMISSION_LEVELS['SENIOR_LEADER'] or
            can_manage_assets(user_role) or
            has_management_access(user_role)
        )
        ...
        if not has_broad_access:
            # Limited access: only their own assignments
            qs = qs.filter(employee=user)
    ...
    def perform_update(self, serializer):
        ...
        for asset in removed_assets:
            # Only set AVAILABLE if asset is not part of any other assignments
            still_assigned_elsewhere = AssetAssignment.objects.filter(
                assets=asset
...
class AssetHistoryViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        has_broad_access = (
            user.is_superuser or
            user.is_staff or
            user.has_perm('assets.view_assethistory') or
            permission_level >= PERMISSION_LEVELS['SENIOR_LEADER'] or
            can_manage_assets(user_role) or
            has_management_access(user_role)
        )
        ...
        # Limited access: only history they performed
        return qs.filter(performed_by=user)
...
class OffboardingAssetReturnViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        has_broad_access = (
            user.is_superuser or
            user.is_staff or
            user.has_perm('assets.view_offboardingassetreturn') or
            permission_level >= PERMISSION_LEVELS['SENIOR_LEADER'] or
            can_manage_assets(user_role) or
            has_management_access(user_role)
        )
        ...
        # Limited access: only their own returns
        return qs.filter(user=user)
...
class AssetReturnViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        has_broad_access = (
            user.is_superuser or
            user.is_staff or
            user.has_perm('assets.view_assetreturn') or
            permission_level >= PERMISSION_LEVELS['SENIOR_LEADER'] or
            can_manage_assets(user_role) or
            has_management_access(user_role)
        )
        ...
        # Limited access: only returns for their assignments
        return qs.filter(assignment__employee=user)
...
class EmployeeStatusViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        has_broad_access = (
            user.is_superuser or
            user.is_staff or
            user.has_perm('assets.view_employeestatus') or
            permission_level >= PERMISSION_LEVELS['SENIOR_LEADER'] or
            can_manage_assets(user_role) or
            has_management_access(user_role)
        )
        ...
        # Limited access: only their own status
        return qs.filter(employee=user)
...
class AssetRepairViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Asset team members see all repairs for their asset types
        if user.email in AssetType.objects.values_list('asset_team_email', flat=True):
            return qs
        
        # Regular users see only repairs for their assigned assets
        return qs.filter(asset__assignments__employee=user).distinct()
    ...
    def perform_create(self, serializer):
        ...
        # Notify Admins, IT Support, and Asset Team
        NotificationService.notify_asset_repair_request(repair)
