...
#     user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='asset_offboarding_records')
...
# Auto return all assets when an offboarding record is marked as offboarded
@receiver(post_save, sender=OffboardingAssetReturn)
def auto_return_assets_on_offboarding(sender, instance, created, **kwargs):
    try:
        # Only proceed if flagged as offboarded
        if not instance.is_offboarded:
            return
        ...
        # Collect all current assignments for this user
        assignments = AssetAssignment.objects.filter(employee=user)
        returned_ids = []

        for assignment in assignments:
            # Assets currently linked to this assignment
            for asset in list(assignment.assets.all()):
                # Skip if already has a return record for this assignment/asset
                if AssetReturn.objects.filter(assignment=assignment, asset=asset).exists():
                    continue

                # Create return record; post_save on AssetReturn will handle status + M2M removal
                AssetReturn.objects.create(
                    assignment=assignment,
                    asset=asset,
                    condition='GOOD',
                    notes=(instance.remarks or 'Auto-returned during offboarding')
                )
                returned_ids.append(asset.id)

        if returned_ids:
            # Record in the offboarding record for traceability
            instance.returned_assets.add(*Asset.objects.filter(id__in=returned_ids))
    except Exception as e:
        # Never block save due to return automation
        print(f"Auto-return on offboarding failed: {e}")