...
# def admin_user_assets_api(request, user_id):
#     try:
#         user = User.objects.get(pk=user_id)
#         assignment = AssetAssignment.objects.filter(employee=user).order_by('-assigned_at').first()
        ...
        if form.is_valid():
            try:
                if employee:
                    ...
                    # Update existing employee record (specific employee link)
                    for field in form.cleaned_data:
                        if field not in ['aadhar_pan_file', 'educational_certificates_file', 'bank_statements_file', 'previous_offer_letter_file',
                                       'payslips_file', 'relieving_experience_letters_file', 'appraisal_hike_letters_file']:
                            setattr(employee, field, form.cleaned_data[field])
                    
                    # Handle file uploads
                    for field_name in ['aadhar_pan_file', 'educational_certificates_file', 'bank_statements_file', 'previous_offer_letter_file',
                                     'payslips_file', 'relieving_experience_letters_file', 'appraisal_hike_letters_file']:
                        if form.cleaned_data[field_name]:
                            setattr(employee, field_name, form.cleaned_data[field_name])
                            # Auto-check corresponding collection field
                    ...
                    # Create new employee record (generic link or direct access)
                    # Check if personal email already exists (including soft-deleted)
                    email = form.cleaned_data['email']
                    existing_employee = Employee.all_objects.filter(email=email).first()
                    ...
                    if existing_employee:
                        if existing_employee.is_deleted:
                            messages.error(request, 
                                'An employee record with this personal email was previously created but has been deactivated. '
                                'Please contact HR for assistance.'
                            )
                            return render(request, 'onboarding/employee_form.html', {
                                'form': form,
                ...
                # Send confirmation email to employee (using personal email)
                try:
                    send_confirmation_email(employee)
                except Exception as e:
                    print(f"Failed to send confirmation email: {str(e)}")
...
Note: HR will complete your employment details (department, position, company email) after reviewing your submission.

Mandatory documents uploaded successfully:
✓ Aadhar & PAN Card
...
HR Team - Techoptima Pvt Ltd"""
    ...
    email = EmailMessage(
        subject=f"Onboarding Information Received - {employee.full_name}",
        body=message,
        to=[employee.email],  # Send to personal email
    )
    email.content_subtype = "plain"
    email.send()
...
def send_hr_notification(employee, is_generic_link, encoded_data, link_created_time):
    ...
    if employee.aadhar_pan_file:
        mandatory_docs.append("✓ Aadhar & PAN Card")
    if employee.educational_certificates_file:
        mandatory_docs.append("✓ Educational Certificates")
    if employee.bank_statements_file:
        mandatory_docs.append("✓ Bank Statements")
    if employee.previous_offer_letter_file:
        mandatory_docs.append("✓ Previous Offer Letter")
    
    optional_docs = []
    if employee.payslips_file:
        optional_docs.append("✓ Payslips")
    if employee.relieving_experience_letters_file:
        optional_docs.append("✓ Relieving & Experience Letters")
    if employee.appraisal_hike_letters_file:
        optional_docs.append("✓ Appraisal/Hike Letters")
...
MANDATORY Documents Uploaded ({len(mandatory_docs)}/4):
{chr(10).join(mandatory_docs) if mandatory_docs else '❌ No mandatory documents uploaded'}

OPTIONAL Documents Uploaded ({len(optional_docs)}/3):
{chr(10).join(optional_docs) if optional_docs else 'No optional documents uploaded'}
