...
class EmployeeForm(forms.ModelForm):
    class Meta:
        model = Employee
        fields = ['first_name', 'last_name', 'email', 'company_email', 'phone_number', 'employee_type', 'department', 'position', 'current_address', 'permanent_address', 'joining_date']
...
class EmployeeSelfOnboardingForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            ...
            'current_address', 'permanent_address',
            # MANDATORY DOCUMENTS
            'aadhar_pan_file', 'educational_certificates_file', 'bank_statements_file', 'previous_offer_letter_file',
            # OPTIONAL DOCUMENTS
            'payslips_file', 'relieving_experience_letters_file', 'appraisal_hike_letters_file'
        ]
        ...
        widgets = {
            ...
            'bank_statements_file': forms.FileInput(attrs={
                'class': 'form-control',
                'accept': '.pdf,.jpg,.jpeg,.png'
            }),
            'previous_offer_letter_file': forms.FileInput(attrs={
        ...
        labels = {
            ...
            'aadhar_pan_file': 'Aadhar & PAN Card (PDF/Image) *',
            'educational_certificates_file': 'Educational Certificates/Degree (PDF/Image) *',
            'bank_statements_file': 'Last 6 Months Bank Statements (PDF/Image) *',
            'previous_offer_letter_file': 'Previous Offer Letter (PDF/Image) *',
            # OPTIONAL DOCUMENTS
            'payslips_file': 'Last 6 Months Payslips (PDF/Image)',
            'relieving_experience_letters_file': 'Relieving & Experience Letters (PDF/Image)',
        ...
        help_texts = {
            'first_name': 'Enter your first name as per official documents',
            'last_name': 'Enter your last name as per official documents',
            'email': 'Use your personal email address (HR will set your company email later)',
            'phone_number': 'Include country code (e.g., +91 9876543210)',
            'current_address': 'Include complete current address with city, state, and PIN code',
            'permanent_address': 'Include complete permanent address with city, state, and PIN code',
            # MANDATORY DOCUMENTS
            'aadhar_pan_file': 'Upload scanned copy of Aadhar and PAN card in single file (REQUIRED)',
            'educational_certificates_file': 'Upload degree certificates and mark sheets (REQUIRED)',
            'bank_statements_file': 'Upload last 6 months bank statements (REQUIRED)',
            'previous_offer_letter_file': 'Upload your previous company offer letter (REQUIRED)',
            # OPTIONAL DOCUMENTS
            'payslips_file': 'Upload last 6 months salary slips (Optional - can be combined in single PDF)',
            'relieving_experience_letters_file': 'Upload relieving letter and experience letter from previous company (Optional)',
            'appraisal_hike_letters_file': 'Upload any appraisal or salary hike letters from previous company (Optional)',
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Make MANDATORY fields required for self-submission
        required_fields = [
            'first_name', 'last_name', 'email', 'phone_number', 
            'current_address', 'permanent_address',
        ...
        optional_fields = ['payslips_file', 'relieving_experience_letters_file', 'appraisal_hike_letters_file']
        for field_name in optional_fields:
            if field_name in self.fields:
                self.fields[field_name].required = False
    
    def clean_email(self):
        """Custom validation for personal email field to handle soft-deleted employees"""
        email = self.cleaned_data.get('email')
        if not email:
            return email
        ...
        if existing_employee:
            if existing_employee.is_deleted:
                raise ValidationError(
                    'An employee record with this personal email was previously created but has been deactivated by HR. '
                    'Please contact HR for assistance.'
                )
            elif existing_employee.is_self_submitted:
                raise ValidationError(
                    'An employee with this personal email has already completed onboarding. '
                    'Please contact HR if you need assistance.'
                )
            else:
                raise ValidationError(
                    'An employee record with this personal email already exists. '
                    'Please contact HR for assistance.'
                )
    ...
    def save(self, commit=True):
        ...
        # Mark as self-submitted and auto-check document collection for uploaded files
        employee.is_self_submitted = True
        
        # Auto-check MANDATORY document collection boxes since files are uploaded
        if employee.aadhar_pan_file:
            employee.aadhar_pan_collected = True
        if employee.educational_certificates_file:
            employee.educational_certificates_collected = True
        if employee.bank_statements_file:
            employee.bank_statements_collected = True
        if employee.previous_offer_letter_file:
            employee.previous_offer_letter_collected = True
            
        # Auto-check OPTIONAL document collection boxes if files are uploaded
        if employee.payslips_file:
            employee.payslips_collected = True
        if employee.relieving_experience_letters_file:
            employee.relieving_experience_letters_collected = True
