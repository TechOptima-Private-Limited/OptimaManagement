...
const OnboardingManagement = () => {
  ...
  const updateEmployee = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/onboarding/employees/${editingEmployee.id}/`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('access_token')}`,
          'Content-Type': 'application/json',
  ...
  const fetchDocumentStatus = async (employeeId) => {
    try {
      const apiBase = process.env.REACT_APP_API_URL || `${window.location.protocol}//${window.location.hostname}:8080/api`;
      const response = await fetch(`${apiBase}/onboarding/employees/${employeeId}/documents_status/`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('access_token')}`,
          'Content-Type': 'application/json',
        },
  ...
  const updateEmployeeStatus = async (employeeId, status) => {
    try {
      const apiBase = process.env.REACT_APP_API_URL || `${window.location.protocol}//${window.location.hostname}:8080/api`;
      const response = await fetch(`${apiBase}/onboarding/employees/${employeeId}/update_status/`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('access_token')}`,
          'Content-Type': 'application/json',
  ...
  const uploadDocuments = async () => {
    ...
    try {
      ...
      const apiBase = process.env.REACT_APP_API_URL || `${window.location.protocol}//${window.location.hostname}:8080/api`;
      const response = await fetch(`${apiBase}/onboarding/employees/${uploadingEmployee.id}/upload_documents/`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('access_token')}`,
        },
