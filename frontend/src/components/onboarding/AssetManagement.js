...
const AssetManagement = () => {
  ...
  const getAssetStatusColor = (status) => {
    ...
    if (s === 'DAMAGED' || s === 'REPAIR') return 'bg-amber-500/10 text-amber-400 border border-amber-500/20';
    if (s === 'LOST') return 'bg-rose-500/10 text-rose-400 border border-rose-500/20';
    if (s === 'DISPOSED') return 'bg-slate-500/10 text-slate-500 dark:text-slate-400 border border-slate-500/20';
    return 'bg-slate-500/10 text-slate-500 dark:text-slate-400 border border-slate-500/20';
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#070B14] text-slate-800 dark:text-slate-100 p-4 lg:p-8 transition-colors duration-500">
      {/* Header */}
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between mb-8 gap-4">
        <div>
          <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white sm:text-5xl mb-2 bg-clip-text text-transparent bg-gradient-to-r from-indigo-600 to-slate-800 dark:from-white dark:to-slate-400">
            Asset Management
          </h1>
        ...
        <button
          ...
          className="inline-flex items-center px-6 py-3 border border-transparent text-sm font-semibold rounded-xl shadow-lg text-slate-900 dark:text-white hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 transform hover:scale-105"
        >
          {exportingExcel ? (
            <>
              <svg className="animate-spin -ml-1 mr-2 h-5 w-5 text-slate-900 dark:text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          ...
          {[
            ...
            return (
              <div
                ...
                className={`text-left rounded-2xl p-6 transition-all duration-300 backdrop-blur-xl border flex flex-col justify-between group h-full ${selected ? 'shadow-[0_0_20px_rgba(99,102,241,0.2)]' : 'hover:bg-black/10 dark:bg-white/10 hover:border-black/20 dark:border-white/20'}`}
              >
                <div className="font-bold text-lg text-slate-900 dark:text-white mb-4 group-hover:text-indigo-400 transition-colors uppercase tracking-wider">{item.label}</div>
                <div className="flex items-center justify-end gap-3">
                  ...
                  <span className={`text-xs font-bold px-3 py-1.5 rounded-lg border transition-all ${selected ? 'bg-indigo-600 text-slate-900 dark:text-white border-indigo-500' : 'bg-black/5 dark:bg-white/5 text-slate-500 dark:text-slate-400 border-black/10 dark:border-white/10 group-hover:border-black/20 dark:border-white/20'}`}>
                    Manage
                  </span>
                </div>
              </div>
      ...
      {isAssetManager && activeSection === 'types' && (
        <div className="max-w-7xl mx-auto bg-black/5 dark:bg-white/5 backdrop-blur-xl rounded-2xl border border-black/10 dark:border-white/10 overflow-hidden shadow-2xl p-6">
          {assetTypes.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-white/10">
                <thead style={{ background: theme.headerGradient }}>
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Name</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Category</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Tag Prefix</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Description</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Team Email</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {assetTypes.map(t => (
                    <tr key={t.id} className="hover:bg-black/5 dark:bg-white/5 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">{(t.category === 'HARDWARE' && 'Hardware') || (t.category === 'SOFTWARE' && 'Software') || t.category}</td>
                      <td className="px-6 py-4 text-sm font-mono text-slate-500">{t.tag_prefix}</td>
                      <td className="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">{t.description || '(None)'}</td>
                      <td className="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">{t.asset_team_email || ''}</td>
        ...
        <div className="max-w-7xl mx-auto">
          {(() => {
            ...
            if (sw.length === 0) return (
              <div className="text-center py-20 bg-black/5 dark:bg-white/5 backdrop-blur-xl rounded-2xl border border-black/10 dark:border-white/10">
                <RectangleStackIcon className="mx-auto h-16 w-16 text-slate-600 mb-4" />
                <h3 className="text-xl font-bold text-slate-900 dark:text-white">No software assets yet</h3>
              </div>
            );
            return (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {sw.map(s => (
                  <div key={s.id} className="bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 backdrop-blur-xl rounded-2xl p-6 hover:bg-black/10 dark:bg-white/10 transition-all group border-l-4 border-l-indigo-500 shadow-xl">
                    ...
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2 group-hover:text-indigo-300 transition-colors">{s.description || s.asset_type}</h3>
                    ...
                    <div className="mt-6 pt-4 border-t border-black/5 dark:border-white/5 flex justify-end">
          ...
          {(() => {
            ...
            if (grouped.length === 0) return (
              <div className="text-center py-20 bg-black/5 dark:bg-white/5 backdrop-blur-xl rounded-2xl border border-black/10 dark:border-white/10">
                ...
                <h3 className="text-xl font-bold text-slate-900 dark:text-white">No active assignments</h3>
              </div>
            );
            return (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {grouped.map(g => {
                  ...
                  return (
                    <div key={g.employee} className="bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 backdrop-blur-xl rounded-2xl p-6 hover:bg-black/10 dark:bg-white/10 transition-all shadow-xl group">
                      <div className="flex items-center gap-4 mb-6">
                        <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-xl font-bold text-slate-900 dark:text-white shadow-lg shadow-indigo-500/20">
                          {emp?.name?.charAt(0) || emp?.username?.charAt(0) || 'E'}
                        </div>
                        <div>
                          <h3 className="text-lg font-bold text-slate-900 dark:text-white group-hover:text-indigo-300 transition-colors">{emp?.name || emp?.username || 'Employee'}</h3>
                          <p className="text-xs text-slate-500 dark:text-slate-400">{g.manager_email || 'General Assignment'}</p>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        {typesLabel.length > 0 && (
                          <div>
                            <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2">Requested Types</p>
                            <div className="flex flex-wrap gap-2">
                              {typesLabel.map((t, idx) => (
                                <span key={idx} className="px-2 py-1 rounded-lg bg-indigo-500/10 text-indigo-400 text-[10px] font-bold border border-black/5 dark:border-white/5">
                                  {t}
                                </span>
                              ))}
                            </div>
                        ...
                        {assetsFiltered.length > 0 && (
                          <div>
                            <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2">Assigned Assets</p>
                            <div className="space-y-2">
                              {assetsFiltered.map((as, idx) => (
                                <div key={idx} className="flex items-center gap-3 p-2 rounded-xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5">
                                  <div className="text-indigo-400 bg-indigo-500/10 p-1.5 rounded-lg">
                                    {getAssetIcon(as.asset_type)}
                                  </div>
                                  <div className="min-w-0">
                                    <p className="text-xs font-semibold text-slate-900 dark:text-white truncate">{as.asset_type}</p>
                        ...
                        {g.notes && (
                          <div className="mt-4 p-3 rounded-xl bg-amber-500/5 border border-amber-500/10">
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed italic line-clamp-2">{g.notes}</p>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          {returnsList.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {returnsList.map(r => {
                ...
                return (
                  <div key={r.id} className="bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 backdrop-blur-xl rounded-2xl p-6 hover:bg-black/10 dark:bg-white/10 transition-all shadow-xl group">
                    <div className="flex justify-between items-start mb-6">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-xl bg-orange-500/10 text-orange-400 flex items-center justify-center text-sm font-bold border border-orange-500/20">
                          {emp?.name?.charAt(0) || 'U'}
                        </div>
                        <div>
                          <h3 className="text-sm font-bold text-slate-900 dark:text-white">{emp?.name || 'User'}</h3>
                          <p className="text-[10px] text-slate-500 uppercase tracking-widest">{new Date(r.created_at || Date.now()).toLocaleDateString()}</p>
                        </div>
                      </div>
                      <span className={`px-2 py-1 rounded-lg text-[10px] font-bold border ${r.is_offboarded ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-amber-500/10 text-amber-400 border-amber-500/20'}`}>
                        {r.is_offboarded ? 'OFFBOARDED' : 'IN PROGRESS'}
                      </span>
                    </div>

                    <div className="space-y-3">
                      <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Returned Assets</p>
                      {labels.map((l, idx) => (
                        <div key={idx} className="flex items-center gap-3 p-2 rounded-xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5">
                          ...
                          <div className="min-w-0">
                            <p className="text-xs font-semibold text-slate-900 dark:text-white truncate">{l.type}</p>
                    ...
                    {r.remarks && (
                      <div className="mt-4 p-3 rounded-xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 italic line-clamp-2">{r.remarks}</p>
                      </div>
                    )}
                  </div>
                );
      ...
      {showAddHardwareModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0">
          <div className="flex items-center justify-center min-h-screen">
            <div className="fixed inset-0 transition-opacity bg-slate-50 dark:bg-[#070B14]/80 backdrop-blur-sm" onClick={() => setShowAddHardwareModal(false)}></div>
            <div className="relative inline-block align-bottom bg-[#0B1120] rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-xl sm:w-full border border-black/10 dark:border-white/10">
              <div className="px-8 pt-8 pb-6">
                <div className="flex justify-between items-center mb-8">
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Add Hardware Asset</h3>
                  <button onClick={() => setShowAddHardwareModal(false)} className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-6">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Asset Type *</label>
                      <select
                        value={hardwareForm.asset_type}
                        onChange={(e) => setHardwareForm({ ...hardwareForm, asset_type: e.target.value })}
                        className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all appearance-none"
                      >
                        <option value="" className="bg-white dark:bg-[#1a1c2e]">Select Type</option>
                        {(assetTypes || [])
                          .filter(t => (t.category === 'HARDWARE' || t.category === 'Hardware'))
                          .map(t => (
                            <option key={t.id} value={t.name} className="bg-white dark:bg-[#1a1c2e]">{t.name}</option>
                          ))}
                      </select>
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Name / Model *</label>
                      <input
                        type="text"
                        placeholder="e.g. MacBook Pro M3"
                        value={hardwareForm.name}
                        onChange={(e) => setHardwareForm({ ...hardwareForm, name: e.target.value })}
                        className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all"
                      />
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Asset Tag</label>
                      <input
                        type="text"
                        placeholder="OPT-HW-001"
                        value={hardwareForm.asset_tag}
                        onChange={(e) => setHardwareForm({ ...hardwareForm, asset_tag: e.target.value })}
                        className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all font-mono"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Serial Number</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Status</label>
                      <select
                        value={hardwareForm.status}
                        onChange={(e) => setHardwareForm({ ...hardwareForm, status: e.target.value })}
                        className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all appearance-none"
                      >
                        <option value="available" className="bg-white dark:bg-[#1a1c2e]">Available</option>
                        <option value="assigned" className="bg-white dark:bg-[#1a1c2e]">Assigned</option>
                        <option value="repair" className="bg-white dark:bg-[#1a1c2e]">Repair</option>
                      </select>
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Purchased Date</label>
                      <input
                        type="date"
                        value={hardwareForm.purchased_date}
                        onChange={(e) => setHardwareForm({ ...hardwareForm, purchased_date: e.target.value })}
                        className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all [color-scheme:dark]"
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-8">
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Custom Attributes / Description</label>
                  <textarea
                    rows={3}
                ...
                <div className="mt-6 flex items-center p-4 bg-black/5 dark:bg-white/5 rounded-2xl border border-black/5 dark:border-white/5">
                  <div className="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:ring-offset-2">
                    ...
                    />
                    <span className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${hardwareForm.is_active ? 'translate-x-5' : 'translate-x-0'}`}></span>
                    <span className={`pointer-events-none absolute inset-0 rounded-full transition-colors duration-200 ease-in-out ${hardwareForm.is_active ? 'bg-indigo-600' : 'bg-slate-200 dark:bg-slate-700'}`}></span>
                  </div>
                  <label className="ml-3 block text-sm font-medium text-slate-700 dark:text-slate-300">Active Asset</label>
                </div>
              </div>

              <div className="px-8 py-6 bg-black/5 dark:bg-white/5 border-t border-black/5 dark:border-white/5 flex flex-col sm:flex-row-reverse gap-3">
                <button
                  onClick={() => submitHardware('save')}
                  disabled={!hardwareForm.asset_type || !hardwareForm.name}
                  className="flex-1 px-6 py-3 bg-gradient-to-r from-indigo-600 to-violet-600 text-slate-900 dark:text-white rounded-xl font-bold shadow-lg shadow-indigo-500/25 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:scale-100"
                >
                  Save Asset
                </button>
                <button
                  onClick={() => setShowAddHardwareModal(false)}
                  className="flex-1 px-6 py-3 bg-black/5 dark:bg-white/5 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:text-white rounded-xl font-bold border border-black/10 dark:border-white/10 hover:bg-black/10 dark:bg-white/10 transition-all"
                >
                  Cancel
                </button>
              </div>
      ...
      {showAddTypeModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0">
          <div className="flex items-center justify-center min-h-screen">
            <div className="fixed inset-0 transition-opacity bg-slate-50 dark:bg-[#070B14]/80 backdrop-blur-sm" onClick={() => setShowAddTypeModal(false)}></div>
            <div className="relative inline-block align-bottom bg-[#0B1120] rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full border border-black/10 dark:border-white/10">
              <div className="px-8 pt-8 pb-6">
                <div className="flex justify-between items-center mb-8">
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{typeForm?.id ? 'Edit Asset Type' : 'Add Asset Type'}</h3>
                  <button onClick={() => setShowAddTypeModal(false)} className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors">
                    <XMarkIcon className="h-6 w-6" />
                  </button>
                </div>
                ...
                <div className="space-y-6">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Type Name *</label>
                    <input
                      type="text"
                      placeholder="e.g. Workstation, Cloud Subscription"
                      value={typeForm.name}
                      onChange={(e) => setTypeForm({ ...typeForm, name: e.target.value })}
                      className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all font-semibold"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Category</label>
                    <select
                      value={typeForm.category}
                      onChange={(e) => setTypeForm({ ...typeForm, category: e.target.value })}
                      className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all appearance-none"
                    >
                      <option value="Hardware" className="bg-white dark:bg-[#1a1c2e]">Hardware</option>
                      <option value="Software" className="bg-white dark:bg-[#1a1c2e]">Software</option>
                      <option value="Other" className="bg-white dark:bg-[#1a1c2e]">Other</option>
                    </select>
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Tag Prefix</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Description</label>
                    <textarea
                      rows={2}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Asset Team Email</label>
                    <input
                      type="email"
                  ...
                  <div className="flex items-center p-4 bg-black/5 dark:bg-white/5 rounded-2xl border border-black/5 dark:border-white/5">
                    <div className="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out">
                      ...
                      />
                      <span className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${typeForm.is_active ? 'translate-x-5' : 'translate-x-0'}`}></span>
                      <span className={`pointer-events-none absolute inset-0 rounded-full transition-colors duration-200 ease-in-out ${typeForm.is_active ? 'bg-indigo-600' : 'bg-slate-200 dark:bg-slate-700'}`}></span>
                    </div>
                    <label className="ml-3 block text-sm font-medium text-slate-700 dark:text-slate-300">Active Type</label>
                  </div>
                </div>
              </div>
      ...
      {showAddSoftwareModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0">
          <div className="flex items-center justify-center min-h-screen">
            <div className="fixed inset-0 transition-opacity bg-slate-50 dark:bg-[#070B14]/80 backdrop-blur-sm" onClick={() => setShowAddSoftwareModal(false)}></div>
            <div className="relative inline-block align-bottom bg-[#0B1120] rounded-3xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full border border-black/10 dark:border-white/10">
              <div className="px-8 pt-8 pb-6">
                <div className="flex justify-between items-center mb-8">
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Add Software Asset</h3>
                  <button onClick={() => setShowAddSoftwareModal(false)} className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors">
                    <XMarkIcon className="h-6 w-6" />
                  </button>
                </div>
                ...
                <div className="space-y-6">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Software Type *</label>
                    <select
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Product Name *</label>
                    <input
                      type="text"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all font-mono text-sm"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">License Key</label>
                      <input
                        type="text"
                  ...
                  <div>
                    ...
                    >
                      <option value="available" className="bg-white dark:bg-[#1a1c2e]">Available</option>
                      <option value="assigned" className="bg-white dark:bg-[#1a1c2e]">Assigned</option>
                      <option value="disabled" className="bg-white dark:bg-[#1a1c2e]">Disabled</option>
                    </select>
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Notes / Terms</label>
                    <textarea
                      rows={3}
                  ...
                  <div className="flex items-center p-4 bg-black/5 dark:bg-white/5 rounded-2xl border border-black/5 dark:border-white/5">
                    <div className="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out">
                      ...
                      />
                      <span className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${softwareForm.is_active ? 'translate-x-5' : 'translate-x-0'}`}></span>
                      <span className={`pointer-events-none absolute inset-0 rounded-full transition-colors duration-200 ease-in-out ${softwareForm.is_active ? 'bg-indigo-600' : 'bg-slate-200 dark:bg-slate-700'}`}></span>
                    </div>
                    <label className="ml-3 block text-sm font-medium text-slate-700 dark:text-slate-300">Active License</label>
                  </div>
                </div>
              </div>
      ...
      {showAssignmentModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0">
          <div className="flex items-center justify-center min-h-screen">
            <div className="fixed inset-0 transition-opacity bg-slate-50 dark:bg-[#070B14]/80 backdrop-blur-sm" onClick={() => setShowAssignmentModal(false)}></div>
            <div className="relative inline-block align-bottom bg-[#0B1120] rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-black/10 dark:border-white/10">
              <div className="px-8 pt-8 pb-6">
                <div className="flex justify-between items-center mb-8">
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Create Assignment</h3>
                  <button onClick={() => setShowAssignmentModal(false)} className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors">
                    <XMarkIcon className="h-6 w-6" />
                  </button>
                </div>
                ...
                <div className="space-y-6">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Employee *</label>
                    <select
                      value={assignmentForm.employee}
                      onChange={(e) => setAssignmentForm({ ...assignmentForm, employee: e.target.value })}
                      className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all appearance-none"
                    >
                      <option value="" className="bg-white dark:bg-[#1a1c2e]">Select Employee</option>
                      {(employees || []).map(emp => (<option key={emp.id} value={emp.id} className="bg-white dark:bg-[#1a1c2e]">{emp.name}</option>))}
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all appearance-none"
                    >
                      <option value="" className="bg-white dark:bg-[#1a1c2e]">Select Asset Type</option>
                      {(assetTypes.length ? assetTypes : []).map(type => (
                        <option key={type.id} value={type.id} className="bg-white dark:bg-[#1a1c2e]">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Available Asset</label>
                    {assignmentForm.isSoftware ? (
                      <div className="mt-1 space-y-2">
                        <div className="max-h-48 overflow-y-auto bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl p-3 scrollbar-none">
                            ...
                            <div className="space-y-1">
                              {(assets || [])
                                ...
                                .map(a => {
                                  ...
                                  return (
                                    <label key={a.id} className="flex items-start gap-3 p-2 hover:bg-black/5 dark:bg-white/5 rounded-lg transition-colors cursor-pointer group">
                                      <div className="relative flex items-center h-5">
                                        <input
                                          ...
                                          className="h-4 w-4 rounded border-black/20 dark:border-white/20 bg-black/5 dark:bg-white/5 text-indigo-600 focus:ring-indigo-500/50 transition-all"
                                        />
                                      </div>
                                      <div className="min-w-0">
                                        <p className="text-sm text-slate-800 dark:text-slate-200 font-medium group-hover:text-slate-900 dark:text-white transition-colors">
                      ...
                      >
                        <option value="" className="bg-white dark:bg-[#1a1c2e]">
                          {!assignmentForm.asset_type
                        ...
                        {(assets || [])
                          ...
                          .map(a => (
                            <option key={a.id} value={a.id} className="bg-white dark:bg-[#1a1c2e]">
                              {a.asset_type} • {a.serial_number || a.description || a.id}
                            </option>
                          ))}
                      </select>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Manager Email</label>
                      <input
                        type="email"
                        ...
                        className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all"
                      />
                    </div>
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Assignment Notes</label>
                    <textarea
                      rows={3}
      ...
      {showReturnModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0">
          <div className="flex items-center justify-center min-h-screen">
            <div className="fixed inset-0 transition-opacity bg-slate-50 dark:bg-[#070B14]/80 backdrop-blur-sm" onClick={() => setShowReturnModal(false)}></div>
            <div className="relative inline-block align-bottom bg-[#0B1120] rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full border border-black/10 dark:border-white/10">
              <div className="px-8 pt-8 pb-6">
                <div className="flex justify-between items-center mb-8">
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Return Asset</h3>
                  <button onClick={() => setShowReturnModal(false)} className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors">
                    <XMarkIcon className="h-6 w-6" />
                  </button>
                </div>
                ...
                <div className="space-y-6">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Asset *</label>
                    <select
                      value={returnForm.asset}
                      onChange={(e) => setReturnForm({ ...returnForm, asset: e.target.value })}
                      className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all appearance-none"
                    >
                      <option value="" className="bg-white dark:bg-[#1a1c2e]">Select Assigned Asset</option>
                      {(assets || [])
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Return Status</label>
                    <select
                      value={returnForm.status}
                      onChange={(e) => setReturnForm({ ...returnForm, status: e.target.value })}
                      className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all appearance-none"
                    >
                      <option value="available" className="bg-white dark:bg-[#1a1c2e]">Available (Good Condition)</option>
                      <option value="damaged" className="bg-white dark:bg-[#1a1c2e]">Damaged / Need Repair</option>
                      <option value="lost" className="bg-white dark:bg-[#1a1c2e]">Lost / Stolen</option>
                      <option value="retired" className="bg-white dark:bg-[#1a1c2e]">Retired / End of Life</option>
                    </select>
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Return Remarks</label>
                    <textarea
                      rows={3}
              ...
              <div className="px-8 py-6 bg-black/5 dark:bg-white/5 border-t border-black/5 dark:border-white/5 flex flex-col sm:flex-row-reverse gap-3">
                <button
                  ...
                  className="flex-1 px-6 py-3 bg-gradient-to-r from-red-600 to-rose-600 text-slate-900 dark:text-white rounded-xl font-bold shadow-lg shadow-red-500/25 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:scale-100"
                >
                  Process Return
                </button>
                <button
...

      {
        showStatusModal && (
          <div className="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0">
            <div className="flex items-center justify-center min-h-screen">
              <div className="fixed inset-0 transition-opacity bg-slate-50 dark:bg-[#070B14]/80 backdrop-blur-sm" onClick={() => setShowStatusModal(false)}></div>
              <div className="relative inline-block align-bottom bg-[#0B1120] rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full border border-black/10 dark:border-white/10">
                <div className="px-8 pt-8 pb-6">
                  <div className="flex justify-between items-center mb-8">
                    <h3 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Add Employee Status</h3>
                    <button onClick={() => setShowStatusModal(false)} className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors">
                      <XMarkIcon className="h-6 w-6" />
                    </button>
                  </div>
                  <div className="space-y-6">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Notes</label>
                      <textarea
                        rows={3}
                ...
                <div className="px-8 py-6 bg-black/5 dark:bg-white/5 border-t border-black/5 dark:border-white/5 flex flex-col sm:flex-row-reverse gap-3">
                  <button onClick={submitStatus} disabled={!statusForm.employee || !statusForm.status} className="flex-1 px-6 py-3 bg-gradient-to-r from-indigo-600 to-violet-600 text-slate-900 dark:text-white rounded-xl font-bold shadow-lg shadow-indigo-500/25 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:scale-100">Add</button>
                  <button onClick={() => setShowStatusModal(false)} className="flex-1 px-6 py-3 bg-black/5 dark:bg-white/5 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:text-white rounded-xl font-bold border border-black/10 dark:border-white/10 hover:bg-black/10 dark:bg-white/10 transition-all">Cancel</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-2 md:grid-cols-6 gap-6 mb-10">
              {[
                ...
                return (
                  <div key={stat.key} className="bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 backdrop-blur-xl rounded-2xl p-6 transition-all duration-300 hover:bg-black/10 dark:bg-white/10 hover:border-black/20 dark:border-white/20 group">
                    <div className="flex flex-col items-center text-center">
                      ...
                      <div>
                        <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-1">{stat.count}</h3>
            ...
            <div className="flex flex-col sm:flex-row gap-6 mb-8 items-end">
              <div className="flex-1">
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 ml-1">Search Assets</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="block w-full pl-12 pr-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl leading-5 text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all backdrop-blur-md"
                  />
                </div>
              </div>
              <div className="w-full sm:w-64">
                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 ml-1">Status Filter</label>
                <select
                  value={filterType}
                  onChange={(e) => setFilterType(e.target.value)}
                  className="block w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all backdrop-blur-md appearance-none"
                >
                  <option value="all" className="bg-white dark:bg-[#1a1c2e]">All Statuses</option>
                  <option value="AVAILABLE" className="bg-white dark:bg-[#1a1c2e]">Available</option>
                  <option value="ASSIGNED" className="bg-white dark:bg-[#1a1c2e]">Assigned</option>
                  <option value="DAMAGED" className="bg-white dark:bg-[#1a1c2e]">Repair</option>
                  <option value="DAMAGED" className="bg-white dark:bg-[#1a1c2e]">Damaged</option>
                  <option value="LOST" className="bg-white dark:bg-[#1a1c2e]">Lost</option>
                </select>
              </div>
            </div>

            <div className="bg-black/5 dark:bg-white/5 backdrop-blur-xl rounded-2xl border border-black/10 dark:border-white/10 overflow-hidden shadow-2xl">
              {filteredAssets.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <p className="mt-2 text-slate-500 dark:text-slate-400 max-w-xs mx-auto">
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-white/10">
                    <thead style={{ background: theme.headerGradient }}>
                      <tr>
                        <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Type</th>
                        <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Asset Tag</th>
                        <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Status</th>
                        <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Employee</th>
                        <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">History</th>
                        <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Serial #</th>
                        <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Description</th>
                        <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Purchased</th>
                        <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Age</th>
                        <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Issued</th>
                        {isAssetAdmin && (
                          <th scope="col" className="px-6 py-4 text-right text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Actions</th>
                        )}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-white/5">
                      {filteredAssets.map((asset) => (
                        <tr
                          key={asset.id}
                          className="hover:bg-black/5 dark:bg-white/5 cursor-pointer transition-colors group/row border-b border-black/5 dark:border-white/5 last:border-0"
                            ...
                            {(() => {
                              ...
                              return prev.length ? (
                                <div className="flex -space-x-2">
                                  {prev.map((p, i) => (
                                    <div key={i} className="h-6 w-6 rounded-full bg-indigo-600 border-2 border-[#070B14] flex items-center justify-center text-[8px] font-bold text-slate-900 dark:text-white shadow-lg" title={p}>
                                      {p.charAt(0)}
                                    </div>
                                  ))}
                                </div>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400">
                            {(() => {
                              ...
                              return age !== null && age !== undefined ? (
                                <span className={`px-2 py-0.5 rounded-lg border border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 text-xs font-bold ${age > 3 ? 'text-orange-400' : 'text-emerald-400'}`}>
                                  {age}y
                                </span>
                              ) : '-';
                            })()}
                            ...
                            >
                              <div className="flex justify-end gap-3">
                                ...
                                <button
                                  ...
                                  className="p-2 text-orange-400 hover:bg-orange-500/20 rounded-xl transition-all"
                                  title="Report Repair"
                                >
              ...
              <div className="flex-1">
                <div className="relative group">
                  ...
                  <input
                    ...
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="block w-full pl-12 pr-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all backdrop-blur-xl"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="bg-black/5 dark:bg-white/5 backdrop-blur-xl rounded-[2.5rem] border border-black/10 dark:border-white/10 overflow-hidden shadow-2xl">
              {filteredAssets.length === 0 ? (
                <div className="text-center py-24">
                  <div className="h-20 w-20 bg-black/5 dark:bg-white/5 rounded-3xl flex items-center justify-center mx-auto mb-6 border border-black/10 dark:border-white/10">
                    <TrashIcon className="h-10 w-10 text-slate-600" />
                  </div>
                  <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">No disposed assets</h3>
                  ...
                  <table className="min-w-full divide-y divide-white/10">
                    <thead style={{ background: theme.headerGradient }}>
                      <tr>
                        <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Type</th>
                        <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Asset Tag</th>
                        <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Serial Number</th>
                        <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Description</th>
                        <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Purchased</th>
                        <th scope="col" className="px-6 py-4 text-right text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Actions</th>
                      </tr>
                    ...
                    <tbody className="divide-y divide-white/5">
                      {filteredAssets.map((asset) => (
                        <tr
                          key={asset.id}
                          className="hover:bg-black/5 dark:bg-white/5 cursor-pointer transition-colors group"
                          onClick={() => openAssetDetails(asset)}
                        >
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-3">
                              <div className="p-2 bg-black/5 dark:bg-white/5 rounded-xl text-slate-500 dark:text-slate-400 group-hover:text-indigo-400 transition-colors">
                                {getAssetIcon(asset.asset_type)}
                              </div>
                              <span className="text-sm font-bold text-slate-900 dark:text-white">{asset.asset_type.replace('_', ' ')}</span>
                            </div>
                          </td>
...

      {
        showDetailsModal && detailsAsset && (
          <div className="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0">
            <div className="flex items-center justify-center min-h-screen">
              <div className="fixed inset-0 transition-opacity bg-slate-50 dark:bg-[#070B14]/90 backdrop-blur-md" onClick={() => setShowDetailsModal(false)}></div>
              <div className="relative inline-block align-bottom bg-[#0B1120] rounded-[2.5rem] text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border border-black/10 dark:border-white/10">
                ...
                <div className="h-32 bg-gradient-to-br from-indigo-600/20 via-violet-600/20 to-transparent relative overflow-hidden">
                  ...
                  <div className="absolute top-8 right-8">
                    <button onClick={() => setShowDetailsModal(false)} className="p-3 bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:bg-white/10 rounded-2xl text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-all backdrop-blur-xl border border-black/10 dark:border-white/10">
                      <XMarkIcon className="h-6 w-6" />
                    </button>
                  </div>
                  <div className="absolute bottom-6 left-10">
                    <div className="flex items-center gap-4">
                      <div className="p-3 bg-black/5 dark:bg-white/5 backdrop-blur-xl rounded-2xl border border-black/10 dark:border-white/10">
                        {getAssetIcon(detailsAsset.asset_type, 'h-8 w-8 text-indigo-400')}
                      </div>
                      <div>
                        <h2 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">{detailsAsset.asset_type}</h2>
                  ...
                  <div className="grid grid-cols-2 gap-10">
                    ...
                    <div className="space-y-8">
                      <div>
                        ...
                        <div className="space-y-5">
                          <div className="flex justify-between items-center group">
                            <span className="text-slate-500 dark:text-slate-400 text-sm">Serial Number</span>
                            <span className="text-slate-900 dark:text-white font-mono text-sm bg-black/5 dark:bg-white/5 px-2 py-0.5 rounded border border-black/5 dark:border-white/5">{detailsAsset.serial_number || 'N/A'}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-slate-500 dark:text-slate-400 text-sm">Model/Description</span>
                            <span className="text-slate-900 dark:text-white text-sm font-semibold">{detailsAsset.description || 'N/A'}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-slate-500 dark:text-slate-400 text-sm">Category</span>
                            <span className="px-2.5 py-1 bg-black/5 dark:bg-white/5 rounded-lg text-xs font-bold text-indigo-300 border border-black/10 dark:border-white/10">{detailsAsset.category}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                          <div className="p-4 bg-black/5 dark:bg-white/5 rounded-2xl border border-black/5 dark:border-white/5 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              <span className="text-slate-800 dark:text-slate-200 font-bold uppercase text-xs tracking-widest">{detailsAsset.status}</span>
                            </div>
                    ...
                    <div className="space-y-8">
                      <div>
                        ...
                        <div className="p-6 bg-black/5 dark:bg-white/5 rounded-[2rem] border border-black/10 dark:border-white/10 relative group overflow-hidden">
                          ...
                          {detailsAsset.employee_name ? (
                            <div className="relative">
                              <div className="flex items-center gap-4 mb-4">
                                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-500 flex items-center justify-center text-slate-900 dark:text-white font-bold text-xl shadow-lg shadow-indigo-500/20">
                                  {detailsAsset.employee_name.charAt(0)}
                                </div>
                                <div>
                                  <p className="text-slate-900 dark:text-white font-bold text-lg">{detailsAsset.employee_name}</p>
                              ...
                              <div className="space-y-2 pt-4 border-t border-black/5 dark:border-white/5">
                                <div className="flex justify-between text-xs">
                                  ...
                                  <span className="text-slate-700 dark:text-slate-300 font-mono italic">{formatDateTime(detailsAsset.assigned_date)}</span>
                                </div>
                              </div>
                            </div>
                          ) : (
                            <div className="flex flex-col items-center justify-center py-4 text-center">
                              <div className="w-16 h-16 rounded-full bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 flex items-center justify-center mb-3">
                      ...
                      {detailsAsset.custom_attributes && (
                        <div>
                          ...
                          <div className="p-4 bg-black/5 dark:bg-white/5 rounded-2xl border border-black/5 dark:border-white/5 text-slate-700 dark:text-slate-300 text-sm leading-relaxed italic">
                            "{detailsAsset.custom_attributes}"
                          </div>
                        </div>
                      )}
                  ...
                  <div className="mt-12 pt-8 border-t border-black/5 dark:border-white/5 flex justify-between items-center">
                    ...
                    <div className="flex gap-3">
                      <button
                        ...
                        className="px-6 py-2.5 bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:bg-white/10 rounded-xl text-slate-900 dark:text-white font-bold border border-black/10 dark:border-white/10 transition-all flex items-center gap-2"
                      >
                      ...
                      <button
                        ...
                        className="px-8 py-2.5 bg-gradient-to-r from-indigo-600 to-violet-600 rounded-xl text-slate-900 dark:text-white font-bold shadow-lg shadow-indigo-500/20 hover:scale-[1.02] transition-all"
                      >
                        Close
                      </button>
                    </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0">
          <div className="flex items-center justify-center min-h-screen">
            <div className="fixed inset-0 transition-opacity bg-slate-50 dark:bg-[#070B14]/80 backdrop-blur-sm" onClick={() => setShowCreateModal(false)}></div>
            <div className="relative inline-block align-bottom bg-[#0B1120] rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-black/10 dark:border-white/10">
              <div className="px-8 pt-8 pb-6">
                <div className="flex justify-between items-center mb-8">
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Add New Asset</h3>
                  <button onClick={() => setShowCreateModal(false)} className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors">
                    <XMarkIcon className="h-6 w-6" />
                  </button>
                </div>
                <div className="space-y-6">
                  <div>
                    ...
                    >
                      ...
                      {(employees || []).map((employee) => (
                        <option key={employee.id} value={employee.id} className="bg-white dark:bg-[#1a1c2e]">
                          {employee.name}
                        </option>
                      ))}
                    </select>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all appearance-none"
                    >
                      <option value="laptop" className="bg-white dark:bg-[#1a1c2e]">Laptop</option>
                      <option value="phone" className="bg-white dark:bg-[#1a1c2e]">Phone</option>
                      <option value="id_card" className="bg-white dark:bg-[#1a1c2e]">ID Card</option>
                      <option value="access_card" className="bg-white dark:bg-[#1a1c2e]">Access Card</option>
                      <option value="other" className="bg-white dark:bg-[#1a1c2e]">Other</option>
                    </select>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all font-mono"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Issued Date</label>
                      <input
                        type="date"
              ...
              <div className="px-8 py-6 bg-black/5 dark:bg-white/5 border-t border-black/5 dark:border-white/5 flex flex-col sm:flex-row-reverse gap-3">
                <button
                  ...
                  className="flex-1 px-6 py-3 bg-gradient-to-r from-indigo-600 to-violet-600 text-slate-900 dark:text-white rounded-xl font-bold hover:scale-[1.02] transition-all disabled:opacity-50"
                >
                  Create Asset
                </button>
                <button
                  onClick={() => setShowCreateModal(false)}
                  className="flex-1 px-6 py-3 bg-black/5 dark:bg-white/5 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:text-white rounded-xl font-bold border border-black/10 dark:border-white/10 transition-all"
                >
                  Cancel
                </button>
              </div>
      ...
      {showEditModal && selectedAsset && (
        <div className="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0">
          <div className="flex items-center justify-center min-h-screen">
            <div className="fixed inset-0 transition-opacity bg-slate-50 dark:bg-[#070B14]/80 backdrop-blur-sm" onClick={() => setShowEditModal(false)}></div>
            <div className="relative inline-block align-bottom bg-[#0B1120] rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-black/10 dark:border-white/10">
              <div className="px-8 pt-8 pb-6">
                <div className="flex justify-between items-center mb-8">
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Edit Asset</h3>
                  <button onClick={() => setShowEditModal(false)} className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors">
                    <XMarkIcon className="h-6 w-6" />
                  </button>
                </div>
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      >
                        <option value="AVAILABLE" className="bg-white dark:bg-[#1a1c2e]">Available</option>
                        <option value="ASSIGNED" className="bg-white dark:bg-[#1a1c2e]">Assigned</option>
                        <option value="DAMAGED" className="bg-white dark:bg-[#1a1c2e]">Repair / Damaged</option>
                        <option value="LOST" className="bg-white dark:bg-[#1a1c2e]">Lost</option>
                      </select>
                    </div>
                  </div>
              ...
              <div className="px-8 py-6 bg-black/5 dark:bg-white/5 border-t border-black/5 dark:border-white/5 flex flex-col sm:flex-row-reverse gap-3">
                <button
                  onClick={updateAsset}
                  className="flex-1 px-6 py-3 bg-gradient-to-r from-indigo-600 to-violet-600 text-slate-900 dark:text-white rounded-xl font-bold hover:scale-[1.02] transition-all"
                >
                  Update Asset
                </button>
                <button
      ...
      {showRepairModal && repairForm.asset && (
        <div className="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0">
          <div className="flex items-center justify-center min-h-screen">
            <div className="fixed inset-0 transition-opacity bg-slate-50 dark:bg-[#070B14]/80 backdrop-blur-sm" onClick={() => setShowRepairModal(false)}></div>
            <div className="relative inline-block align-bottom bg-[#0B1120] rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-black/10 dark:border-white/10">
              <div className="px-8 pt-8 pb-6">
                <div className="flex justify-between items-center mb-8">
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{repairForm.id ? 'Edit Asset Repair' : 'Report Asset Repair'}</h3>
                  <button onClick={() => setShowRepairModal(false)} className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors">
                    <XMarkIcon className="h-6 w-6" />
                  </button>
                </div>
                ...
                <div className="space-y-6">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Asset</label>
                    <div className="p-4 bg-black/5 dark:bg-white/5 rounded-2xl border border-black/5 dark:border-white/5 text-sm">
                      <div className="text-indigo-400 font-mono font-bold mb-1">{repairForm.asset.asset_tag}</div>
                      <div className="text-slate-700 dark:text-slate-300">{repairForm.asset.description || repairForm.asset.name}</div>
                    </div>
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Issue Description *</label>
                    <textarea
                      rows={3}
                      ...
                      className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all font-medium"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-4 py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all appearance-none"
                      >
                        <option value="PENDING" className="bg-white dark:bg-[#1a1c2e]">Pending</option>
                        <option value="PICKED_UP" className="bg-white dark:bg-[#1a1c2e]">Picked Up</option>
                        <option value="AT_VENDOR" className="bg-white dark:bg-[#1a1c2e]">At Vendor</option>
                        <option value="IN_PROGRESS" className="bg-white dark:bg-[#1a1c2e]">In Progress</option>
                        <option value="COMPLETED" className="bg-white dark:bg-[#1a1c2e]">Completed</option>
                        <option value="FAILED" className="bg-white dark:bg-[#1a1c2e]">Failed</option>
                        <option value="CANCELLED" className="bg-white dark:bg-[#1a1c2e]">Cancelled</option>
                      </select>
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Estimated Completion</label>
                      <input
                        type="date"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Vendor</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Repair Cost (₹)</label>
                      <input
                        type="number"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 ml-1">Repair Notes</label>
                    <textarea
                      rows={2}
              ...
              <div className="px-8 py-6 bg-black/5 dark:bg-white/5 border-t border-black/5 dark:border-white/5 flex flex-col sm:flex-row-reverse gap-3">
                <button
                  ...
                  className="flex-1 px-6 py-3 bg-gradient-to-r from-orange-600 to-amber-600 text-slate-900 dark:text-white rounded-xl font-bold shadow-lg shadow-orange-500/25 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:scale-100"
                >
