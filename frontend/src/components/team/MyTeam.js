  ...
  return (
    <div className="min-h-screen bg-[#070B14] py-8 transition-colors duration-500">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        {teamSize > 0 && (
          <>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="bg-white/5/5 backdrop-blur-xl rounded-2xl border border-black/10 dark:border-white/10 p-6 shadow-xl group hover:bg-black/10 dark:bg-white/5/10 transition-all duration-300 relative overflow-hidden">
                ...
                {attendanceStats.onLeave.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {attendanceStats.onLeave.map(member => (
                      <div key={member.id} className="flex items-center space-x-3 p-2 bg-white/5/5 rounded-xl border border-white/10 group-hover:border-black/20 dark:border-white/20 transition-all">
                  ...
                  <div className="py-6 text-center bg-white/5/5 rounded-2xl border border-dashed border-black/10 dark:border-white/10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
              <div className="bg-white/5/5 backdrop-blur-xl rounded-2xl border border-black/10 dark:border-white/10 p-6 shadow-xl flex items-center justify-between group hover:bg-black/10 dark:bg-white/5/10 transition-all duration-300">
                <div>
            ...
            <div className="space-y-6 pt-4">
              ...
              <div className="bg-white/5/5 backdrop-blur-xl rounded-[2.5rem] border border-black/10 dark:border-white/10 overflow-hidden shadow-2xl">
                <div className="px-8 py-6 flex items-center justify-between border-b border-black/10 dark:border-white/10 bg-white/5/5">
                  <button
                    onClick={() => navigateMonth(-1)}
                    className="p-3 rounded-2xl bg-white/5/5 border border-black/10 dark:border-white/10 text-slate-400 hover:bg-black/10 dark:bg-white/5/10 hover:text-white transition-all shadow-lg active:scale-95"
                  >
                ...
                {calendarLoading ? (
                  ...
                  <div className="overflow-x-auto custom-scrollbar">
                    <table className="w-full border-collapse">
                      <thead>
                        <tr className="bg-white/5/5 border-b border-black/10 dark:border-white/10">
                      ...
                      <tbody className="divide-y divide-white/5">
                        {team.map(member => (
                          <tr key={member.id} className="hover:bg-white/5/5 transition-colors group">
                                          ...
                                          status === 'ABSENT' ? 'bg-rose-500 text-white ring-2 ring-rose-500/50 shadow-rose-500/20' :
                                            status === 'HALF_DAY' ? 'bg-rose-500/50 text-white ring-2 ring-rose-500/30' :
                                              status === 'WEEKOFF' ? 'bg-slate-200 dark:bg-slate-700/50 text-slate-400 opacity-40 shadow-none' :
                                                'bg-white/5/5 text-slate-600 border border-white/10 opacity-20 shadow-none'}
                              `}>
                                    {status === 'PRESENT' ? 'P' :
                                      status === 'WFH' ? 'W' :
                                        status === 'LEAVE' ? 'L' :
                ...
                <div className="px-8 py-5 bg-white/5/5 border-t border-black/10 dark:border-white/10 flex flex-wrap gap-x-10 gap-y-4 items-center">
                  ...
                  <div className="flex items-center space-x-3 group">
                    <div className="w-5 h-5 bg-slate-200 dark:bg-slate-700/50 border border-black/10 dark:border-white/10 rounded-md flex items-center justify-center text-[8px] font-black text-slate-400 group-hover:scale-125 transition-transform">•</div>
        ...
        {/* Team Members List */}
        {teamSize > 0 && (
          <div className="bg-white/5/5 backdrop-blur-xl rounded-3xl border border-black/10 dark:border-white/10 p-8 shadow-2xl relative overflow-hidden group">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 space-y-6 md:space-y-0 relative z-10">
              <h2 className="text-3xl font-black text-white tracking-tight flex items-center">
                ...
                <span className="ml-4 px-3 py-1 bg-black/10 dark:bg-white/5/10 rounded-xl text-sm font-black text-indigo-400 border border-black/10 dark:border-white/10">{teamSize}</span>
              </h2>
              <div className="relative w-full md:w-80">
                ...
                <input
                  ...
                  className="block w-full pl-12 pr-4 py-3 border border-black/10 dark:border-white/10 rounded-[1.25rem] leading-5 bg-white/5/5 placeholder-slate-500 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500/50 text-sm transition-all shadow-inner"
                />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
              {team.filter(member => {
                ...
                <div
                  key={member.id}
                  className="flex items-center space-x-5 p-6 bg-white/5/5 rounded-[2rem] border border-white/10 hover:bg-black/10 dark:bg-white/5/10 hover:border-black/20 dark:border-white/20 hover:scale-[1.02] transform transition-all duration-300 group shadow-lg"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {member.department && (
                      <div className="mt-3 inline-block px-2 py-0.5 bg-white/5/5 border border-black/10 dark:border-white/10 rounded-lg text-[10px] font-black text-slate-500 uppercase tracking-widest">
        ...
        {teamSize === 0 && (
          <div className="bg-white/5/5 backdrop-blur-xl rounded-[3rem] border border-black/10 dark:border-white/10 p-24 shadow-2xl text-center relative overflow-hidden group">
        ...
        {selectedCategory && (
          <div className="fixed inset-0 z-[150] flex justify-end">
            ...
            <div className="relative bg-[#070B14] w-full max-w-[95%] md:max-w-6xl h-full shadow-2xl border-l border-black/10 dark:border-white/10 flex flex-col animate-slide-in-right">
              <div className="px-8 py-6 border-b border-black/10 dark:border-white/10 flex items-center justify-between bg-white/5/5 backdrop-blur-xl">
                <div>
                ...
                <button
                  type="button"
                  onClick={closePanel}
                  className="p-2 rounded-xl bg-white/5/5 border border-black/10 dark:border-white/10 text-slate-400 hover:text-white hover:bg-black/10 dark:bg-white/5/10 transition-all shadow-lg active:scale-95"
                >
              ...
              <div className="flex-1 overflow-hidden flex flex-col">
                <div className="p-8 pb-0">
                  <div className="mb-8 relative">
                    ...
                    <input
                      ...
                      className="block w-full pl-12 pr-4 py-3 border border-black/10 dark:border-white/10 rounded-2xl leading-5 bg-white/5/5 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500/50 text-sm transition-all shadow-inner"
                    />
                  </div>
                </div>

                <div className="flex-1 overflow-auto custom-scrollbar px-4 pb-6">
                  <div className="bg-white/5/5 rounded-[2rem] border border-black/10 dark:border-white/10 overflow-hidden shadow-2xl">
                    {(selectedCategory === 'late' || selectedCategory === 'on_time') ? (
                      <div className="overflow-x-auto custom-scrollbar">
                        <table className="w-full table-fixed border-collapse">
                          <thead>
                            <tr className="bg-white/5/5 border-b border-black/10 dark:border-white/10 text-left">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-8">
                        {getFilteredTeamMembers().map((member) => (
                          <div key={member.id} className="flex items-center space-x-5 p-6 bg-white/5/5 rounded-3xl border border-white/10 hover:bg-black/10 dark:bg-white/5/10 hover:border-black/20 dark:border-white/20 transition-all group shadow-xl">
                ...
                <div className="px-3 py-2 bg-white/5/5 border-t border-black/10 dark:border-white/10 flex justify-between items-center mt-auto backdrop-blur-xl">
                  ...
                  <div className="flex items-center space-x-3">
                    <div className="flex space-x-2">
                      <button className="p-2 rounded-xl bg-white/5/5 border border-black/10 dark:border-white/10 text-slate-600 hover:text-white transition-all disabled:opacity-30 disabled:hover:bg-transparent" disabled>
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 19l-7-7 7-7" /></svg>
                      </button>
                      <span className="text-[11px] font-black text-white bg-indigo-500/80 px-4 py-2 rounded-xl shadow-lg shadow-indigo-500/20 border border-black/10 dark:border-white/10 flex items-center">PAGE 1 OF 1</span>
                      <button className="p-2 rounded-xl bg-white/5/5 border border-black/10 dark:border-white/10 text-slate-600 hover:text-white transition-all disabled:opacity-30 disabled:hover:bg-transparent" disabled>
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 5l7 7-7 7" /></svg>
                      </button>
                    </div>
