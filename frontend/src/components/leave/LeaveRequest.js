...
import { isHRManager, getCurrentUser } from '../../utils/auth';
...
const LeaveRequest = () => {
  ...
  const fetchLeaveBalances = useCallback(async () => {
    try {
      ...
      const currentEmployeeId = user?.employee_id || user?.employee_pk || user?.id;
      const params = { year: new Date().getFullYear() };

      if (isHRManager() && currentEmployeeId) {
        params.employee_id = currentEmployeeId;
      }

      const response = await leaveAPI.initializeMyBalances(params);
      ...
      try {
        ...
        const response = await leaveAPI.getLeaveBalances(params);
  ...
  const fetchLeaveRequests = useCallback(async () => {
    try {
      ...
      if (effectiveEmployeeId) {
        params.employee_id = effectiveEmployeeId;
      } else if (isHRManager()) {
        const user = getCurrentUser();
        const currentEmployeeId = user?.employee_id || user?.employee_pk || user?.id;
  ...
  useEffect(() => {
    ...
    fetchLeaveTypes();
    fetchLeaveBalances();
  }, [fetchLeaveRequests, fetchLeaveTypes, fetchLeaveBalances]);
  ...
  }, [startDate, endDate, leaveDuration, setValue]);

  useEffect(() => {
    if (startDate && endDate && leaveDuration) {
      calculateDays();
    }
  }, [startDate, endDate, leaveDuration, calculateDays]);
  ...
  const onSubmit = async (data) => {
    ...
    if (!validateLeaveBalance(leaveTypeId, daysRequested)) {
      const availableBalance = getAvailableBalance(leaveTypeId);
  ...
  return (
    <div className="space-y-8 relative z-10">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Leave Requests</h3>
          ...
          <button
            ...
            className={`inline-flex items-center px-6 py-3 bg-gradient-to-r ${theme.primaryGradient} text-white font-bold rounded-2xl shadow-lg border ${theme.cardBorder} transition-all transform hover:scale-105`}
          >
      ...
      {leaveBalances.length > 0 && (
        <div className={`${theme.cardBg} border ${theme.cardBorder} rounded-[2rem] p-8 shadow-2xl relative overflow-hidden backdrop-blur-xl`}>
          <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/5 to-purple-500/5"></div>
          <div className="flex items-center mb-6 relative z-10">
            <div className="p-3 bg-indigo-500/20 border border-indigo-500/30 rounded-2xl shadow-lg">
              <SparklesIcon className="h-8 w-8 text-indigo-400" />
            </div>
            <h4 className="text-xl font-bold text-slate-900 dark:text-white ml-4 tracking-wide">Your Leave Balance</h4>
          </div>
            ...
            {leaveBalances.map((balance, idx) => {
              ...
              return (
                  <div
                    ...
                    className={`${theme.muted.bg} p-4 rounded-[2rem] border ${theme.cardBorder} shadow-lg hover:shadow-indigo-500/10 transition-all hover:-translate-y-1 ${accent.glow}`}
                  >
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <div className="min-w-0">
                      ...
                      <button
                        ...
                        className={`text-[12px] font-bold ${theme.info.text} hover:opacity-80 whitespace-nowrap`}
                        title="View details"
                      >
                      View details
                    </button>
                  ...
                  <div className="relative w-28 h-28 mx-auto">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-center">
                        <div className="text-xl font-black text-slate-900 dark:text-white">{formatDays(remaining)}</div>
                  ...
                  <div className={`mt-5 grid grid-cols-2 gap-y-4 gap-x-3 pt-5 border-t ${theme.cardBorder}`}>
                    <div>
                      <div className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest">{isUnpaid ? 'TYPE' : 'AVAILABLE'}</div>
                      <div className="text-sm font-black text-slate-900 dark:text-white mt-1">{isUnpaid ? 'Unpaid (LOP)' : dayLabel(remaining)}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest">CONSUMED</div>
                      <div className="text-sm font-black text-slate-900 dark:text-white mt-1">{dayLabel(used)}</div>
                    </div>
                    <div className="col-span-1">
                      <div className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest">{isUnpaid ? 'STATUS' : 'ACCRUED SO FAR'}</div>
                      <div className="text-sm font-black text-slate-900 dark:text-white mt-1">{isUnpaid ? 'Unlimited' : dayLabel(accruedSoFar)}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest">ANNUAL QUOTA</div>
                      <div className="text-sm font-black text-slate-900 dark:text-white mt-1">{isUnpaid ? 'Unlimited' : dayLabel(annualQuota)}</div>
                    </div>
                  </div>
                </div>
              );
      ...
      {showBalanceDetails && (
        <div className={`fixed inset-0 z-[60] ${theme.isDark ? 'bg-black/60' : 'bg-slate-900/40'} backdrop-blur-md`}>
          <div className="h-full overflow-auto px-4 py-8">
            <div className={`max-w-6xl mx-auto ${theme.modalBg} border ${theme.cardBorder} rounded-[2.25rem] shadow-2xl overflow-hidden`}>
              <div className={`flex items-center justify-between px-6 py-4 border-b ${theme.cardBorder}`}>
                ...
                <button
                  ...
                  className={`inline-flex items-center justify-center w-10 h-10 rounded-2xl ${theme.muted.bg} border ${theme.cardBorder} text-slate-900 dark:text-white hover:bg-black/10 dark:bg-white/10 transition-all`}
                  aria-label="Close"
                  title="Close"
                >
                  X
              ...
              <div className="grid grid-cols-1 md:grid-cols-[240px_1fr] h-full">
                {/* Left nav */}
                <div className={`border-b md:border-b-0 md:border-r ${theme.cardBorder} ${theme.muted.bg}`}>
                  <div className="p-4">
                    {leaveBalances.map((b) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left px-4 py-3 rounded-2xl border transition-all mb-2 ${isActive
                            ? `${theme.info.bg} border-indigo-500/20 text-slate-900 dark:text-white`
                            : `bg-transparent border-transparent text-slate-500 dark:text-slate-400 hover:${theme.muted.bg} hover:${theme.cardBorder}`
                            }`}
                        >
                ...
                <div className="p-6">
                  <div className="flex items-center gap-4 mb-5">
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl font-bold border transition-all ${balanceDetailsTab === 'history'
                        ? `${theme.info.bg} ${theme.info.border} ${theme.info.text}`
                        : `${theme.muted.bg} ${theme.cardBorder} text-slate-500 dark:text-slate-400 hover:bg-black/10 dark:bg-white/10`
                        }`}
                    >
                      Balance history
                    </button>
                  ...
                  {balanceDetailsLoading ? (
                    <div className="py-16 text-center text-slate-500 dark:text-slate-400 font-bold">Loading…</div>
                  ) : balanceDetailsTab === 'history' ? (
                    <div className={`${theme.muted.bg} border ${theme.cardBorder} rounded-2xl overflow-hidden`}>
                      <div className={`px-5 py-4 border-b ${theme.cardBorder} flex items-center justify-between`}>
                        <div className="text-slate-900 dark:text-white font-black">Transactions</div>
                        <div className="text-slate-500 dark:text-slate-400 text-sm font-bold">Year {balanceDetailsData?.year ?? ''}</div>
                      </div>
                      <div className="overflow-x-auto">
                        <table className="min-w-full">
                          <thead>
                            <tr className="text-left text-slate-500 dark:text-slate-400 text-xs font-black uppercase tracking-widest">
                          ...
                          <tbody className={`divide-y ${theme.cardBorder}`}>
                            {(balanceDetailsData?.entries ?? []).length === 0 ? (
                              ...
                              (balanceDetailsData?.entries ?? []).map((e) => (
                                <tr key={e.id}>
                                  <td className="px-5 py-4 text-slate-900 dark:text-white font-bold">
                                    {formatDate(e.transaction_date, 'dd MMM yyyy')}
                                  </td>
                                  <td className="px-5 py-4 font-black">
                                    <span className={Number(e.change) >= 0 ? `${theme.success.text}` : `${theme.danger.text}`}>
                                      {Number(e.change) >= 0 ? '+' : '-'}
                                      {formatDays(Math.abs(Number(e.change)))}
                                    </span>
                                  </td>
                                  <td className="px-5 py-4 text-slate-900 dark:text-white font-bold">{formatDays(e.balance)}</td>
                    ...
                    <div className={`${theme.muted.bg} border ${theme.cardBorder} rounded-2xl p-5`}>
                      <div className="text-slate-900 dark:text-white font-black mb-3">Leave policy</div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className={`${theme.cardBg} border ${theme.cardBorder} rounded-xl p-4`}>
                          <div className="text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Annual quota</div>
                        ...
                        <div className={`${theme.cardBg} border ${theme.cardBorder} rounded-xl p-4`}>
                          <div className="text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Carry forward</div>
                          <div className="text-lg font-black text-slate-900 dark:text-white mt-2">
                            {(balanceDetailsData?.policy?.carry_forward_enabled ? 'Enabled' : 'No')}
                          </div>
                        </div>
                      </div>
                      <div className="mt-4 text-slate-500 dark:text-slate-400 text-sm font-medium">
      ...
      <div className={`${theme.cardBg} backdrop-blur-xl p-6 rounded-[2rem] border ${theme.cardBorder} shadow-2xl`}>
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-2">From Date</label>
            <input
              type="date"
          ...
          <div>
            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-2">To Date</label>
            <input
              type="date"
      ...
      <div className={`${theme.cardBg} backdrop-blur-xl shadow-2xl rounded-[2rem] border ${theme.cardBorder} overflow-hidden`}>
        <ul className={`divide-y ${theme.cardBorder}`}>
          {leaveRequests.length === 0 ? (
            <li className="p-16 text-center relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col items-center space-y-6">
                ...
                <div>
                  <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">No Leave Requests</h3>
            ...
            leaveRequests.map((request) => (
              <li key={request.id}>
                <div className={`px-8 py-6 hover:${theme.muted.bg} transition-all`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-6">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm text-slate-500 dark:text-slate-400 font-medium mb-2">
                          {isHRManager() && (
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2">
            ...
            <div>
              ...
              <input
                ...
                step="0.5"
                readOnly
                className={`block w-full ${theme.muted.bg} border ${theme.cardBorder} rounded-xl text-slate-500 dark:text-slate-400 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 font-medium cursor-not-allowed`}
              />
            </div>
            ...
            <div className="sm:col-span-2">
              ...
              <div className={`mt-2 flex justify-center px-6 pt-5 pb-6 border-2 border-dashed ${theme.cardBorder} ${theme.muted.bg} rounded-2xl p-8 text-center hover:border-indigo-400/50 transition-colors`}>
                <DocumentArrowUpIcon className="mx-auto h-16 w-16 text-slate-500" />
                <div className="mt-4">
                  <label className={`cursor-pointer ${theme.info.bg} hover:bg-indigo-500/30 border ${theme.info.border} ${theme.info.text} px-6 py-3 rounded-xl font-bold transition-all inline-block hover:-translate-y-0.5 shadow-lg`}>
                  ...
                  <p className="mt-4 text-sm text-slate-500 dark:text-slate-400 font-medium">PNG, JPG, PDF up to 10MB</p>
            ...
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                rows={4}
                className={`block w-full ${theme.muted.bg} border ${theme.cardBorder} rounded-xl text-slate-900 dark:text-white shadow-sm focus:ring-indigo-500 focus:border-indigo-500 font-medium placeholder-slate-500 p-4`}
          ...
          <div className={`flex justify-end space-x-4 pt-8 border-t ${theme.cardBorder} mt-8`}>
            <button
              type="button"
              onClick={() => setShowRequestModal(false)}
              className={`px-8 py-3 ${theme.muted.bg} hover:bg-black/10 dark:bg-white/10 text-slate-900 dark:text-white font-bold rounded-2xl border ${theme.cardBorder} transition-all`}
            >
              Cancel
            </button>
            <button
              ...
              className="px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-slate-900 dark:text-white font-bold rounded-2xl shadow-[0_0_15px_rgba(79,70,229,0.4)] border border-indigo-400/50 transition-all transform hover:scale-105 disabled:opacity-50 disabled:transform-none"
            >
      ...
      >
        {selectedRequest && (
          <div className="space-y-8">
            {/* Employee Info */}
            {isHRManager() && (
              <div className={`${theme.muted.bg} p-6 rounded-2xl border ${theme.cardBorder} shadow-lg`}>
                <div className="flex items-center space-x-4">
                  <div className={`h-16 w-16 rounded-2xl ${theme.info.bg} border ${theme.info.border} flex items-center justify-center shadow-[0_0_15px_rgba(79,70,229,0.3)]`}>
                    <span className={`font-black text-xl ${theme.info.text}`}>
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400">{selectedRequest.employee?.employee_id}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className={`${theme.muted.bg} border ${theme.cardBorder} p-5 rounded-2xl shadow-inner`}>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Leave Type</label>
                <p className="text-lg font-bold text-slate-900 dark:text-white">{selectedRequest.leave_type?.name}</p>
              </div>
              <div className={`${theme.muted.bg} border ${theme.cardBorder} p-5 rounded-2xl shadow-inner`}>
              ...
              <div className={`${theme.muted.bg} border ${theme.cardBorder} p-5 rounded-2xl shadow-inner`}>
                ...
                <p className="text-lg font-bold text-slate-900 dark:text-white">{formatDate(selectedRequest.start_date)}</p>
              </div>
              <div className={`${theme.muted.bg} border ${theme.cardBorder} p-5 rounded-2xl shadow-inner`}>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">End Date</label>
                <p className="text-lg font-bold text-slate-900 dark:text-white">{formatDate(selectedRequest.end_date)}</p>
              </div>
              <div className={`${theme.muted.bg} border ${theme.cardBorder} p-5 rounded-2xl shadow-inner`}>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Days Requested</label>
                <p className="text-lg font-bold text-slate-900 dark:text-white">{selectedRequest.days_requested} days</p>
              </div>
              <div className={`${theme.muted.bg} border ${theme.cardBorder} p-5 rounded-2xl shadow-inner`}>
            ...
            {/* Reason */}
            <div>
              <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">Reason</label>
              <div className={`${theme.info.bg} p-6 rounded-2xl border ${theme.info.border} shadow-inner`}>
            ...
            {selectedRequest.employee_comments && (
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">Employee Comments</label>
            ...
            {selectedRequest.manager_comments && (
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">Manager Comments</label>
            ...
            {selectedRequest.approved_by_name && (
              <div className={`${theme.muted.bg} p-6 rounded-2xl border ${theme.cardBorder} shadow-lg flex justify-between items-center`}>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Approved By</label>
                  <p className="text-lg font-bold text-slate-900 dark:text-white">{selectedRequest.approved_by_name}</p>
                </div>
            ...
            {selectedRequest.supporting_document && (
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">Supporting Document</label>
                <div className="mt-2">
                  <a
