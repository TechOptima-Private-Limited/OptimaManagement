...
import { employeeAPI, attendanceAPI, leaveAPI, workFromHomeAPI, holidayAPI, API_BASE_URL } from '../../services/api';
...
const Dashboard = () => {
  ...
  // import shared base URL so we don't diverge from the axios service configuration
  // (Dashboard.js is not part of the service layer, so we do the import inside the component)

  // computeBase placeholder removed - using shared constant
  ...
  const fetchAllHolidays = async () => {
    try {
      // holidayAPI already uses axios with API_BASE_URL, so nothing to change here
