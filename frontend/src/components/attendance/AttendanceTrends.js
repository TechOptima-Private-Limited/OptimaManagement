...
const AttendanceTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-slate-900/95 border border-indigo-500/30 rounded-xl p-3 shadow-2xl backdrop-blur-xl">
...
const KpiCard = ({ label, value, suffix = '', icon: Icon, color, trend }) => (
  <div className={`relative bg-white/5 border border-white/10 rounded-2xl p-5 overflow-hidden group hover:border-${color}-500/40 transition-all duration-300`}>
...
const AttendanceTrends = ({ theme, attendanceRecords = [], isManagementRole = false }) => {
  ...
  const chartData = apiData?.trend?.length ? apiData.trend : localTrend;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            disabled={loading}
            className="p-2 rounded-xl bg-white/5 border border-white/10 text-slate-400 hover:text-white hover:border-indigo-500/40 transition-all"
            title="Refresh data"
          >
      ...
      {/* ── Charts ── */}
      <div className={`bg-white/5 border border-white/10 rounded-2xl p-6 shadow-xl backdrop-blur-sm ${loading ? 'opacity-60' : ''}`}>
        {loading && (
              ...
              return (
                <div className="space-y-1.5">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xs font-bold text-white">
                      {avgPct > 0 ? `${Math.round((totalLate / (chartData.length * (avgPct / 100))) * 10)}%` : '—'} avg per period
                    </span>
                  </div>
                </div>
              );
