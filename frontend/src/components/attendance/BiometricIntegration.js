...
const BiometricIntegration = () => {
  ...
  if (!(isHRManager() || isAdmin())) {
    return (
      <div className="min-h-screen bg-slate-950 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-900/20 via-slate-900 to-black text-slate-700 dark:text-slate-300 flex items-center justify-center -mt-16">
        <div className="text-center py-12 px-6 bg-white dark:bg-slate-900/60 backdrop-blur-xl border border-black/10 dark:border-white/10 rounded-2xl shadow-2xl max-w-md w-full">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Only HR Managers or Admins can access biometric integration.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Sync attendance from ZK biometric devices directly</p>
          </div>
          <button
            onClick={() => {
              setEditingDevice(null);
            ...
            className="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-indigo-500 to-purple-600 border border-indigo-500/50 rounded-xl shadow-lg text-sm font-bold text-slate-900 dark:text-white hover:from-indigo-400 hover:to-purple-500 transition-all duration-300 transform hover:-translate-y-0.5"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          {[
            ...
            <div key={label} className="bg-white dark:bg-slate-900/60 backdrop-blur-xl border border-black/10 dark:border-white/10 rounded-2xl p-6 hover:bg-white dark:bg-slate-900/80 transition-all group">
        ...
        <div className="mb-6 flex items-center space-x-4 bg-white dark:bg-slate-900/50 border border-black/10 dark:border-white/10 rounded-xl px-5 py-3">
          ...
          <input
            ...
            className="bg-black/30 border border-black/10 dark:border-white/10 rounded-lg text-slate-900 dark:text-white px-3 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500/50 [color-scheme:dark]"
          />
          <span className="text-xs text-slate-500 italic">Used for manual sync. Auto-sync always uses today.</span>
        </div>

        {/* Devices List */}
        <div className="bg-white dark:bg-slate-900/60 backdrop-blur-xl shadow-xl overflow-hidden rounded-2xl border border-black/10 dark:border-white/10 mb-8">
          <div className="px-6 py-5 border-b border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5">
          ...
          {devices.length === 0 ? (
            <div className="text-center py-16">
              ...
              <h3 className="text-lg font-medium text-slate-700 dark:text-slate-300">No devices registered</h3>
              ...
              <button
                ...
                className="mt-6 inline-flex items-center px-5 py-2.5 border border-transparent text-sm font-bold rounded-xl text-slate-900 dark:text-white bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-400 hover:to-purple-500 transition-all"
              >
                <ComputerDesktopIcon className="h-5 w-5 mr-2" />
                Add Device
              </button>
            ...
            <div className="divide-y divide-white/5">
              {devices.map(device => {
                ...
                return (
                  <div key={device.id} className="p-6 hover:bg-black/5 dark:bg-white/5 transition-colors group">
                    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">

                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <h4 className="text-lg font-bold text-slate-900 dark:text-white group-hover:text-indigo-300 transition-colors uppercase tracking-tight">{device.device_name}</h4>
                        ...
                        <div className="mt-2 grid grid-cols-2 gap-x-8 gap-y-2 text-sm text-slate-500 dark:text-slate-400">
                          <div><span className="font-semibold text-slate-500 mr-1">ID:</span><span className="font-mono text-slate-700 dark:text-slate-300 text-xs bg-black/20 px-1.5 py-0.5 rounded">{device.device_id}</span></div>
                          <div><span className="font-semibold text-slate-500 mr-1">Status:</span><span className="text-slate-700 dark:text-slate-300">{getDeviceStatusText(device)}</span></div>
                          <div><span className="font-semibold text-slate-500 mr-1">IP:</span><span className="font-mono text-slate-700 dark:text-slate-300 text-xs bg-black/20 px-1.5 py-0.5 rounded">{device.ip_address}</span></div>
                      ...
                      <div className="flex flex-col gap-3 min-w-[230px]">
                        <button
                          ...
                          disabled={isSyncing}
                          className="inline-flex items-center justify-center px-4 py-2 border border-black/10 dark:border-white/10 text-sm font-semibold rounded-xl text-slate-700 dark:text-slate-300 bg-black/20 hover:bg-black/10 dark:bg-white/10 hover:text-slate-900 dark:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          {isSyncing ? (
                        ...
                        <div className="flex items-center gap-2">
                          <select
                            ...
                            className="flex-1 bg-black/30 border border-black/10 dark:border-white/10 rounded-lg text-slate-700 dark:text-slate-300 text-xs px-2 py-1.5 focus:ring-2 focus:ring-indigo-500/50 disabled:opacity-50"
                          >
        ...
        {showAddDevice && (
          <div className="fixed inset-0 z-50 overflow-y-auto">
            <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center">
              <div className="fixed inset-0 bg-white dark:bg-slate-900/90 backdrop-blur-sm" onClick={() => {
                setShowAddDevice(false);
                setEditingDevice(null);
                setNewDevice({ device_name: '', device_id: '', location: '', ip_address: '' });
              }} />
              <div className="inline-block align-bottom bg-[#0A0F1A] border border-black/10 dark:border-white/10 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full relative z-10">
                    ...
                    {[
                      ...
                      <div key={key}>
                        ...
                        <input
                          ...
                          className="block w-full bg-black/20 border border-black/10 dark:border-white/10 rounded-xl py-2.5 px-3 text-slate-900 dark:text-white placeholder-slate-500 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50"
                ...
                <div className="bg-black/5 dark:bg-white/5 border-t border-black/10 dark:border-white/10 px-6 py-4 flex flex-row-reverse gap-3">
                  <button type="submit" form="addDeviceForm" className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-sm font-bold text-slate-900 dark:text-white hover:from-indigo-400 hover:to-purple-500 transition-all">
                  ...
                  <button type="button" onClick={() => {
                    ...
                    setEditingDevice(null);
                    setNewDevice({ device_name: '', device_id: '', location: '', ip_address: '' });
                  }} className="px-6 py-2.5 rounded-xl border border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-black/10 dark:bg-white/10 transition-all">
                    Cancel
                  </button>
                </div>
              </div>
