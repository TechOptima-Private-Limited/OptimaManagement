            ...
            <form onSubmit={handleApprovalSubmit(handleApprovalAction)}>
              <div className="mb-6">
                <div className="flex items-center space-x-4 mb-4">
                  <div className="h-12 w-12 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center">
                    <span className="text-white font-semibold">
                      {selectedApproval.employee_name?.split(' ').map(n => n[0]).join('')}
                    </span>
                  </div>
                  <div>
...
//     const stats = {
//       totalDays: approvedRecords.length,
//       presentDays: approvedRecords.filter(r => r.status === 'PRESENT').length,
//       absentDays: approvedRecords.filter(r => r.status === 'ABSENT').length,
//       lateDays: approvedRecords.filter(r => r.status === 'LATE').length
//     };
//     setStats(stats);
//   };
...
//   const approval = {
//     id: record.id,
//     employee_name: record.employee?.user?.first_name + ' ' + record.employee?.user?.last_name,
//     employee_id: record.employee?.employee_id,
//     date: record.date,
//     edit_reason: record.edit_reason,
...
//                   <h1 className="text-3xl font-bold">Attendance Tracker</h1>
//                   <p className="text-blue-100 mt-1">
//                     {isHRManager() ? 'Manage attendance for all employees with smart insights' : 'Track your daily attendance and performance'}
//                   </p>
//                 </div>
//               </div>
...
//         {/* Employee Pending Edit Requests Section */}
//         {!isHRManager() && (
//           <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-8">
//             <h3 className="text-xl font-semibold text-gray-900 mb-6 flex items-center">
//               <ClockIcon className="h-6 w-6 mr-2 text-amber-500" />
...
//                     <div className="flex justify-between">
//                       <span className="text-red-700 font-medium">Status:</span>
//                       <span className="text-red-900 font-semibold">{record.original_status || 'Not set'}</span>
//                     </div>
//                     <div className="flex justify-between">
//                       <span className="text-red-700 font-medium">Notes:</span>
...
//                 {/* REQUESTED VALUES (What Employee Wants) */}
//                 <div className="bg-green-50 border border-green-200 rounded-xl p-4">
//                   <h5 className="text-sm font-semibold text-green-800 mb-3 flex items-center">
//                     <CheckCircleIcon className="h-4 w-4 mr-1" />
//                     Employee's Requested Changes
...
//         {/* Mark/Edit Attendance Form - Only for Employees */}
//         {!isHRManager() && (
//           <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-8">
//             <div className="flex items-center justify-between mb-6">
//               <div className="flex items-center space-x-3">
...
//                   type="submit"
//                   disabled={submitting}
//                   className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white px-8 py-3 rounded-xl font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
//                 >
//                   {submitting ? (
//                     <>
//                       <LoadingSpinner size="small" />
...
//                 <XCircleIcon className="h-6 w-6 text-red-600" />
//               )}
//               <span>{selectedApproval?.action === 'approve' ? 'Approve Attendance Edit' : 'Reject Attendance Edit'}</span>
//             </div>
//           }
//         > */}
//           {/* {selectedApproval && (
...
//                     <p className="text-sm text-blue-800">
//                       <span className="font-semibold">Reason:</span> "{selectedApproval.edit_reason}"
//                     </p>
//                   </div>
//                 )}
//               </div>
...
//               {selectedApproval.action === 'approve' ? (
//                 <div className="space-y-4">
//                   <p className="text-sm text-gray-700 mb-4 bg-green-50 p-3 rounded-xl border border-green-200">
//                     💡 Review and modify the attendance data as needed, then approve the request.
//                   </p>
...
//                         <XCircleIcon className="h-5 w-5" />
//                       )}
//                       <span>{selectedApproval.action === 'approve' ? 'Approve Edit' : 'Reject Edit'}</span>
//                     </>
//                   )}
//                 </button>
//                 <button
...
//         {/* Floating Action Button for HR - Quick Access to Approvals */}
//         {isHRManager() && attendanceRecords.filter(r => r.is_pending_approval).length > 0 && (
//           <div className="fixed bottom-6 right-6 z-50">
//             <button
//               onClick={() => {
//                 document.querySelector('[data-approvals-section]')?.scrollIntoView({ behavior: 'smooth' });
