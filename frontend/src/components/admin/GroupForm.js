...
const GroupForm = () => {
  ...
  return (
    <div className="min-h-screen bg-[#070B14] dark:bg-[#070B14] p-8 space-y-8">
      ...
      <form onSubmit={handleSubmit} className="bg-white/5/5 backdrop-blur-xl rounded-3xl border border-white/10 shadow-2xl overflow-hidden">
        <div className="p-8 space-y-8">
          <div className="max-w-xl space-y-2">
            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Group Title</label>
            <input
              type="text"
              className="block w-full bg-white/5/5 border border-white/10 rounded-2xl px-5 py-4 text-sm text-white placeholder-slate-600 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all shadow-inner font-bold"
              value={name}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* Available */}
              <div className="bg-white/5/5 rounded-3xl border border-white/10 overflow-hidden flex flex-col">
                <div className="px-6 py-4 bg-white/5/5 border-b border-white/10 flex items-center justify-between">
                ...
                <div className="p-4 bg-white/5/5 border-b border-white/10">
                  <div className="relative">
                    <MagnifyingGlassIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-500" />
                    <input
                      type="text"
                      placeholder="Filter nodes..."
                      className="block w-full bg-white/5/5 border border-white/10 rounded-xl pl-10 pr-4 py-2 text-xs text-white placeholder-slate-600 focus:ring-2 focus:ring-indigo-500/50 transition-all font-medium"
                ...
                <div className="flex-1 max-h-96 overflow-y-auto custom-scrollbar divide-y divide-white/5">
                  {filteredAvailable.map((p) => {
                    ...
                    return (
                      <div key={p.id} className="group flex items-center px-6 py-3 hover:bg-white/5/5 transition-colors cursor-pointer" onClick={() => handleMove('available', [p.id])}>
