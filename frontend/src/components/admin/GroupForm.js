...
const GroupForm = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#070B14] p-8 space-y-8">
      <div className="flex items-center justify-between">
        <div className="space-y-2">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tight">{isEdit ? 'Configure Group' : 'Initialize Group'}</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-black/5 dark:bg-white/5 backdrop-blur-xl rounded-3xl border border-black/5 dark:border-white/5 shadow-2xl overflow-hidden">
        <div className="p-8 space-y-8">
          <div className="max-w-xl space-y-2">
            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Group Title</label>
            <input
              type="text"
              className="block w-full bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 rounded-2xl px-5 py-4 text-sm text-slate-900 dark:text-white placeholder-slate-600 focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all shadow-inner font-bold"
              value={name}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* Available */}
              <div className="bg-black/5 dark:bg-white/5 rounded-3xl border border-black/5 dark:border-white/5 overflow-hidden flex flex-col">
                <div className="px-6 py-4 bg-black/5 dark:bg-white/5 border-b border-black/5 dark:border-white/5 flex items-center justify-between">
                  <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Available Registry</span>
                ...
                <div className="p-4 bg-black/5 dark:bg-white/5 border-b border-black/5 dark:border-white/5">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="block w-full bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 rounded-xl pl-10 pr-4 py-2 text-xs text-slate-900 dark:text-white placeholder-slate-600 focus:ring-2 focus:ring-indigo-500/50 transition-all font-medium"
                ...
                <div className="flex-1 max-h-96 overflow-y-auto custom-scrollbar divide-y divide-white/5">
                  {filteredAvailable.map((p) => {
                    ...
                    return (
                      <div key={p.id} className="group flex items-center px-6 py-3 hover:bg-black/5 dark:bg-white/5 transition-colors cursor-pointer" onClick={() => handleMove('available', [p.id])}>
                        <div className="flex-1">
                          <div className="text-[11px] font-bold text-slate-800 dark:text-slate-200 group-hover:text-indigo-400 transition-colors uppercase tracking-tight">{label}</div>
              ...
              <div className="bg-indigo-500/5 rounded-3xl border border-indigo-500/20 overflow-hidden flex flex-col">
                <div className="px-6 py-4 bg-indigo-500/10 border-b border-indigo-500/20 flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-widest bg-indigo-500 px-2 py-0.5 rounded-full">{filteredChosen.length} ACTIVE</span>
                </div>
                <div className="p-4 bg-black/5 dark:bg-white/5 border-b border-black/5 dark:border-white/5">
                  <div className="relative">
                    <MagnifyingGlassIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-500" />
                  ...
                  {filteredChosen.map((p) => {
                    ...
                    return (
                      <div key={p.id} className="group flex items-center px-6 py-3 hover:bg-red-500/5 transition-colors cursor-pointer" onClick={() => handleMove('chosen', [p.id])}>
                        <div className="flex-1">
                          <div className="text-[11px] font-bold text-slate-900 dark:text-white group-hover:text-red-400 transition-colors uppercase tracking-tight">{label}</div>
