...
// compute base URL from environment or derive from host; strip trailing slash so
// consumers don't accidentally end up with double slashes when appending paths.
// if a specific backend url is provided via .env use it,
// otherwise fall back to a *relative* path which works with CRA proxy
// and avoids hard-coded ports that trigger ad‑blockers
export const API_BASE_URL = ('http://192.168.1.3:8080/api').replace(/\/$/, '');
